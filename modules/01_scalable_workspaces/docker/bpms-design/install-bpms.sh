...
then
  ...
  touch $BPMS_DATA_DIR/$MAVEN_DIR/settings.xml
...
then
  ...
  echo "joe=5f2a6743273e88b9ce310c9271f45dd5" >> $SERVER_INSTALL_DIR/$SERVER_NAME/standalone/configuration/application-users.properties
  echo "alice=bf65c65bc69732cc54d4849c718c2039" >> $SERVER_INSTALL_DIR/$SERVER_NAME/standalone/configuration/application-users.properties
fi
...
then
  ...
  echo "joe=analyst,developer,kie-server,kiemgmt" >> $SERVER_INSTALL_DIR/$SERVER_NAME/standalone/configuration/application-roles.properties
  echo "alice=admin,analyst,developer,rest-all,kiemgmt" >> $SERVER_INSTALL_DIR/$SERVER_NAME/standalone/configuration/application-roles.properties
...
VARS=( MYSQL_MODULE_NAME MYSQL_DRIVER_JAR MYSQL_DRIVER_JAR_DIR BPMS_DB )
for i in "${VARS[@]}"
do
